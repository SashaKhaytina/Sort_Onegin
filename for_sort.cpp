...
struct Text
{
    char* text;
    int len_text;
}; // TODO: PointerArr members should be in Text also
...
int main()
{
    ...
    murderous_sort(pointer_arr.ind_arr, pointer_arr.len_strings, sizeof(char*), murderous_compare_str_end_elem);
    print_sorted_text(&pointer_arr, file);
...
void read_file(char* file_name, Text* onegin, PointerArr* pointer_arr)
{
    ...
    //char*** ind_arr  = &(pointer_arr->ind_arr); // TOD: I don't like this


    FILE* file = fopen(file_name, "r");
    if (file == NULL) printf("Не удалось открыть файл"); // TODO: wtf, why you continue working with file if it's null

    // Узнаем размер файла // TOD: move this to function
    // if (fseek(file, 0, SEEK_END) != 0) printf("Курсор не подвинулся\n"); // TOD: delete this checking
    // *len_text = ftell(file);
    ...
    pointer_arr->len_strings = file_strings_counter(file);
...
// int str_len (char* str1) // TOD: wtf (strlen())
// {
//     assert(str1);
...
//     while ((real_str1[ind_elem_1] != '\0') && (real_str2[ind_elem_2] != '\0')) // TOD: for can do your code smaller and clearer than while
//     {
...
//     int ind_elem_1 = str_len(real_str1) - 1;
//     int ind_elem_2 = str_len(real_str2) - 1;

//     while ((ind_elem_1 != 0) && (ind_elem_2 != 0)) // TOD: for can do your code smaller and clearer than while
//     {
//         // Смещение счетчика до буквы в str1 и str2
...
//     while ((str1[ind_elem_1] != '\0') && (str2[ind_elem_2] != '\0')) // TOD: for can do your code smaller and clearer than while
//     {
//         // Смещение счетчика до буквы в str1 и str2
